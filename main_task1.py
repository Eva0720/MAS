...
generator = Model(inputs=[input1,input2], outputs=[outputs_1,outputs_2,outputs_3,outputs_4,out_residual1,out_residual2,out_residual3,out_residual4])
opt = Adam(lr=0.00005)
...
generator.compile(loss=['mse','mse','mse','mse','mse','mse','mse','mse'], optimizer=opt)


def adjustData(img1,img2,mask1,mask2,mask3,mask4,mask5,mask6,mask7,mask8):
        img1 = img1 / 255  
        img2 = img2 / 255  
        mask1 = mask1 / 255  
        mask2 = mask2 / 255 
        ...
        mask7 = mask7 / 255 
        mask8 = mask8 / 255 
        return (img1,img2,mask1,mask2,mask3,mask4,mask5,mask6,mask7,mask8)
        

def trainGenerator(batch_size,path1,path2,path3,path4,path5,path6,path7,path8,path9,path10, aug_dict,target_size = (256,256),seed = 1):
    ...
    mask7_datagen = ImageDataGenerator(**aug_dict)
    mask8_datagen = ImageDataGenerator(**aug_dict)   
    ...
    mask7_generator = mask7_datagen.flow_from_directory(
         path9,
         class_mode=None,
         batch_size = batch_size,
        # target_size=(128, 128),
    ...
    mask8_generator = mask8_datagen.flow_from_directory(
         path10,
         class_mode=None,
         batch_size = batch_size,
        # target_size=(128, 128),
    ...
    train_generator = zip(image1_generator,image2_generator, mask1_generator,mask2_generator,mask3_generator, mask4_generator,mask5_generator,mask6_generator,mask7_generator,mask8_generator)
    for (img1,img2,mask1,mask2,mask3,mask4,mask5,mask6,mask7,mask8) in train_generator:
        img1,img2,mask1,mask2,mask3,mask4,mask5,mask6,mask7,mask8 = adjustData(img1,img2,mask1,mask2,mask3,mask4,mask5,mask6,mask7,mask8)
        yield ([img1,img2],[mask1,mask2,mask3,mask4,mask5,mask6,mask7,mask8])
...
data_gen_args = dict(
                    ...
                    fill_mode='nearest')
#data_gen_args = dict()
path_auto='/data_task1/Training/input_auto/'
path_dapi='/data_task1/Training/input_dapi/'
path_cd3='/data_task1/Training/output_cd3/' 
path_cd20='/data_task1/Training/output_cd20/' 
path_foxp3='/data_task1/Training/output_foxp3/'  
path_pd1='/data_task1/Training/output_pd1/' 
path_residual_cd3='/data_task1/Training/out_residual_cd3/' 
path_residual_cd20='/data_task1/Training/out_residual_cd20/' 
path_residual_foxp3='/data_task1/Training/out_residual_foxp3/'        
path_residual_pd1='/data_task1/raining/out_residual_pd1/'                             
myGene = trainGenerator(4,path_auto,path_dapi,path_cd3,path_cd20,path_foxp3,path_pd1,path_residual_cd3,path_residual_cd20,path_residual_foxp3,path_residual_pd1,data_gen_args)             

data1_gen_args = dict()


pathv_auto='/data_task1/Testing/input_auto/'
pathv_dapi='/data_task1/Testing/input_dapi/'
pathv_cd3='/data_task1/Testing/output_cd3/' 
pathv_cd20='/data_task1/Testing/output_cd20/'
pathv_foxp3='/data_task1/Testing/output_foxp3/'  
pathv_pd1='/data_task1/Testing/output_pd1/' 
pathv_residual_cd3='/data_task1/Testing/out_residual_cd3/'
pathv_residual_cd20='/data_task1/Testing/out_residual_cd20/'  
pathv_residual_foxp3='/data_task1/Testing/out_residual_foxp3/'         
pathv_residual_pd1='/data_task1/Testing/out_residual_pd1/'    
myGene_val = trainGenerator(4,pathv_auto,pathv_dapi,pathv_cd3,pathv_cd20,pathv_foxp3,pathv_pd1,pathv_residual_cd3,pathv_residual_cd20,pathv_residual_foxp3,pathv_residual_pd1,data1_gen_args)   
...
best_model_path='/models/model_task1.h5'
mc = ModelCheckpoint(best_model_path, monitor='val_loss', save_best_only=True, mode='min')     

generator.fit(myGene,steps_per_epoch=26563//4,validation_data = myGene_val,validation_steps = 2917//4, epochs=200,verbose=1,callbacks=[EarlyStop,Reduce,mc])
